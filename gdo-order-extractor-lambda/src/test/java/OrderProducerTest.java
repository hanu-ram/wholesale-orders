...
class OrderProducerTest {
    ...
    @Test
    @Order(1)
    void shouldPublishDataToWholesaleTopic() throws Exception {
    ...
    @Test
    @Order(2)
    void shouldPublishDataToWholesaleOrderTopicWithMultipleRecords() throws Exception {
    ...
    @Test
    @Order(3)
    void shouldPublishDataToWholesaleErrorTopicForException() throws Exception {
        ...
        String orderDetails = "{\n" +
                ...
                "  \"rejection_reason\": \"J0\"\n" +
                "}";
        final MockProducer<String, String> mockProducerFor
                = new MockProducer<>(true, new StringSerializer(), new StringSerializer());
        when(Configuration.getKafkaTopic()).thenThrow(RuntimeException.class);

        new OrderDetailsProducer().publishOrderDetailsToKafka(mockProducerFor, orderDetails, "133445566", new ArrayList<>());
        assertThat(mockProducerFor.history()).hasSize(1);

        String response = mockProducerFor.history().get(0).value();
        DocumentContext context = JsonPath.parse(response);
        assertEquals("INVALID_MESSAGE", context.read("$.event_name"));
        assertEquals("ORDER-EXTRACTOR-LAMBDA", context.read("$.event_module"));
    }
}